...
impl eframe::App for BridgeApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        self.process_activity_events(ctx);
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.add_enabled_ui(main_enabled, |ui| {
                ...
                match self.active_tab {
                    ...
                    Tab::Settings => {
                        self.render_settings_tab(ctx, ui);
                    }
                    Tab::NodeGraph => {
                        self.render_node_graph_tab(ui);
                    }
                }
...
impl BridgeApp {
    ...
    fn process_activity_events(&mut self, ctx: &egui::Context) {
            ...
            Ok(m) => {
                ui.horizontal(|ui| {
                    ...
                    let profile_names: Vec<String> =
                        m.device_profiles.values().map(|p| p.name.clone()).collect();
                ...
                ui.horizontal_top(|ui| {
                    ...
                    for p in m.device_profiles.values() {
                        if p.eos_bank_size > bank_size {
                            bank_size = p.eos_bank_size;
                        }
                    }
                    if bank_size == 0 {
                        bank_size = 10;
                    ...
                    for i in 0..bank_size {
                        ...
                        ui.allocate_ui(egui::vec2(column_width, 220.0), |ui| {
                            ui.vertical_centered(|ui| {
                                ...
                                ui.add_sized(
                                    [column_width, 18.0],
                                    egui::Label::new(egui::RichText::new(name).small().strong())
                                        .truncate(),
                                );
    ...
    fn render_settings_tab(&mut self, ctx: &egui::Context, ui: &mut egui::Ui) {
                ...
                egui::Grid::new("midi_grid")
                    ...
                    .show(ui, |ui| {
                        if let Ok(m) = self.midi.lock() {
                            ...
                            ui.vertical(|ui| {
                                let mut unique_devices: std::collections::HashSet<String> =
                                ...
                                if sorted_devices.is_empty() {
                                    ui.label(egui::RichText::new("No devices found").italics());
                                }

                                for device in sorted_devices {
                                    let mut enabled =
                                        self.config_edit.enabled_devices.contains_key(&device);
                                    ...
                                    ui.horizontal(|ui| {
                                        if ui.checkbox(&mut enabled, &device).changed() {
                                            if enabled {
                                                    ...
                                                    .contains_key(&device)
                                                {
                                                    ...
                                                    self.config_edit.enabled_devices.insert(
                                                        device.clone(),
                                                        "profiles/icon_platform_m_plus.json"
                                                            .to_string(),
                                                    );
                                                }
                                            } else {
                                                self.config_edit.enabled_devices.remove(&device);
                                            }
                                        }
                                        ...
                                        if enabled {
                                            let current_profile = self
                                                ...
                                                .unwrap_or_default();
                                            ui.label(
                                                egui::RichText::new(&current_profile)
                                                    .italics()
                                                    .small(),
                                            );
                                            if ui
                                                .button("📂")
                                                .on_hover_text("Change Profile")
                                                .clicked()
                                            {
        ...
        if self.config_edit != self.last_applied_config && self.config_edit.validate().is_ok() {
            if let Err(e) = self
                .tx_system
    ...
    fn render_node_graph_tab(&mut self, ui: &mut egui::Ui) {
        let input_zoom = ui.input(|i| i.zoom_delta());
        self.snarl_zoom_pending *= input_zoom;

        // 1. Gather data (hold lock briefly)
        let (profiles, mut selected) = if let Ok(m) = self.midi.lock() {
            let mut keys: Vec<_> = m.device_profiles.keys().cloned().collect();
            keys.sort();
            let current = self
                .selected_controller_for_graph
                .clone()
                .unwrap_or_default();
            (keys, current)
        } else {
        ...
        let mut trigger_repopulate = false;
        ...
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_salt("graph_controller_selector")
                ...
                .show_ui(ui, |ui| {
                    for p in &profiles {
                        if ui
                            .selectable_value(&mut selected, p.clone(), p.clone())
                            .changed()
                        {
                            trigger_repopulate = true;
                        }
                    }
                });
        ...
        if trigger_repopulate {
            self.selected_controller_for_graph = Some(selected.clone());
            let profile_to_load = if let Ok(m) = self.midi.lock() {
                m.device_profiles.get(&selected).cloned()
            } else {
                None
            };

            if let Some(p) = profile_to_load {
                self.populate_snarl(&p);
            }
        }

        ui.separator();

        // Auto-populate if empty and not already handled
        if self.snarl.nodes().next().is_none() {
            // Logic to auto-select first available or currently selected
            let mut p_to_load = None;
            ...
            // Scope for lock
            {
                if let Ok(m) = self.midi.lock() {
                    if let Some(name) = &self.selected_controller_for_graph {
                        if let Some(p) = m.device_profiles.get(name) {
                            p_to_load = Some(p.clone());
                        }
                    } else if let Some((name, p)) = m.device_profiles.iter().next() {
                        name_to_set = Some(name.clone());
                        p_to_load = Some(p.clone());
                    }
                }
            }
            ...
            if let Some(name) = name_to_set {
                self.selected_controller_for_graph = Some(name);
            }
            if let Some(p) = p_to_load {
                self.populate_snarl(&p);
            }
        }
